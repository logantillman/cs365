...
class GraphicsDriver extends JPanel {
	...
	while (console.hasNextLine()) {
		...
		if (objectName.equals("rectangle") ||
		    ...
		    if (objectName.equals("rectangle")) 
			components.add(new graphics.Rectangle(left, top, width, height, c));
		    else
			...
			if (size.equals("SMALL"))
			    arcsize = Roundtangle.Arcsize.SMALL.getSize();
			else if (size.equals("MEDIUM"))
			    arcsize = Roundtangle.Arcsize.MEDIUM.getSize();
			else
			    arcsize = Roundtangle.Arcsize.LARGE.getSize();
		    }
		    Color c = colorMap.get(color);
